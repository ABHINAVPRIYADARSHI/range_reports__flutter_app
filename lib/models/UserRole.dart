...
class UserRole {
  ...
  return UserRole(
    ...
    userStatus: map['user_status']?.toString()?.toLowerCase() ?? 'pending',
    userCreatedAt: map['user_created_at'] != null 
        ? DateTime.tryParse(map['user_created_at'].toString()) ?? DateTime.now()
        : DateTime.now(),
    role: map['role']?.toString() ?? '',
    roleIsActive: map['role_is_active'] == true || 
                 map['role_is_active'] == 1 || 
                 (map['role_is_active']?.toString().toLowerCase() == 'true'),
    roleCreatedAt: map['role_created_at'] != null
        ? DateTime.tryParse(map['role_created_at'].toString()) ?? DateTime.now()
        : DateTime.now(),
